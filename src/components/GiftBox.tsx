      ...
      {!opened ? (
        ...
        >
          ...
          <div className="relative w-[200px] h-[200px] mx-auto">
            {/* Box body */}
            <motion.div
              className="absolute bottom-0 w-full h-[140px] bg-gradient-to-b from-rose-500 to-pink-600 rounded-lg shadow-2xl"
              animate={opened ? { y: -50, opacity: 0 } : { y: 0 }}
              transition={{ duration: 0.6 }}
            >
              {/* Ribbon vertical */}
              <div className="absolute left-1/2 -translate-x-1/2 w-8 h-full bg-gradient-to-b from-yellow-300 to-amber-400" />
              {/* Ribbon horizontal */}
              <div className="absolute top-1/2 -translate-y-1/2 w-full h-8 bg-gradient-to-r from-yellow-300 to-amber-400" />
            </motion.div>

            {/* Box lid */}
            <motion.div
              className="absolute top-0 w-full h-[60px] bg-gradient-to-b from-rose-400 to-pink-500 rounded-t-lg shadow-lg origin-bottom"
